// iter-4A Phase 6 local test: CxlKvStoreA insert/update/search/remove
...
int main() {
  ...
  // dax — we just want CxlKvStoreA's CoW publish + cache+directory
  ...
  // CxlKvBlockPool (single-host, 256 B blocks, room for 4*B blocks).
  const uint32_t kBlockSize = 256;
  const uint32_t kBlocksPerHost = 4 * B;
  std::size_t pool_bytes = CxlKvBlockPool::bytes_for(kBlocksPerHost,
                                                     kBlockSize, 1);
  void *pool_mem = mmap(nullptr, pool_bytes, PROT_READ | PROT_WRITE,
                        MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  if (pool_mem == MAP_FAILED) return 1;
  CxlKvBlockPool pool;
  if (pool.attach(pool_mem, pool_bytes, kBlocksPerHost, kBlockSize,
                  /*host_id=*/0, /*num_hosts=*/1, /*init=*/true) != 0) {
    return 1;
  }
  ...
  if (store.attach(bucket_mem, B, /*host_id=*/0, /*num_hosts=*/1,
                   /*init_region=*/true, &st, &dir, &cache, &fl, &pool) != 0) {
    return 1;
  }
  ...
  printf("ALL PASS — %u keys insert+search+update+remove on CxlKvStoreA\n",
         (uint32_t)keys.size());
