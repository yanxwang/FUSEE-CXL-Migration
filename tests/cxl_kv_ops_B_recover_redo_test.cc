// Phase 6: CxlKvStoreB::recover_from_oplog replays dangling ops.
//
// Mirror of cxl_kv_ops_C_recover_redo_test.cc but for Option B. Single-host
// scenario (num_hosts=1) sidesteps the inter-host ring entirely; combined
// with recovery_mode_ short-circuit inside dispatch_nowait, the redo path
...
int main(int argc, char **argv) {
  ...
  cxl_region_destroy(&r);
  if (fail == 0) {
    printf("OK: CxlKvStoreB::recover_from_oplog on %s\n", dev);
    return 0;
  }
  return 1;
}
