#!/usr/bin/env bash
# iter-12A post-fix scaling_ycsb sweep on g3+g4. Verifies the Phase 5
# stale-host-1-cached-head fix (commit 2e61487) at full scale, using
# the default scaling_ycsb_spec.md config:
#   - REPS=1 (spec default — single-rep already shows whether collapse exists)
#   - MAX_OPS=200000 (spec default)
#   - TIMEOUT_S=600 (spec default)
#
# Scope: 5 workloads × 7 T × 2 cache × 3 KV × 1 rep = 210 runs.
# Wall-clock estimate ~50-80 min on unloaded g3+g4 (per spec §5).
set -u
...
{
  echo "# iter-12A post-fix scaling_ycsb sweep"
