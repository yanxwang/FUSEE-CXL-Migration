#!/usr/bin/env bash
# iter-11A Phase 2 — G1 hash-diff battery for parallel inval drain.
# Per task plan §C14, must PASS at every (KV × workload).
set -u
...
for wl in $WORKLOADS; do
  for kv in $KV_SIZES; do
    ...
    cookie=$(date +%s%N)
    cell="${wl}_kv${kv}"
    H0_REMOTE=/tmp/iter11A_p2_hd_${cell}_h0.bin
    H1_REMOTE=/tmp/iter11A_p2_hd_${cell}_h1.bin
