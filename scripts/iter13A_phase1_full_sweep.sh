#!/usr/bin/env bash
# iter-13A Phase 1.5 — full scaling_ycsb sweep with HAZARD build.
# 5 wl × 7 T × 2 cache × 3 KV × 1 rep = 210 runs.
set -u
...
BIN=/root/FUSEE_CXL/build-cxl-hazard/tests/protocol_a_ycsb
...
{
  echo "# iter-13A Phase 1.5 sweep — HAZARD read-guard build"
  echo "# workloads=$WORKLOADS"
  echo "# threads=$THREADS"
  echo "# cache_modes=$CACHE_MODES"
  echo "# kv_sizes=$KV_SIZES"
  echo "# reps=$REPS  max_ops=$MAX_OPS  timeout=${TIMEOUT_S}s"
    ...
    for kv in $KV_SIZES; do
      for t in $THREADS; do
        for rep in $(seq 1 $REPS); do
          ...
          wait
          sleep 0.2
          cmd_h0="cd /root/FUSEE_CXL/build-cxl-hazard && $COMMON_ENV FUSEE_NUM_HOSTS=2 FUSEE_HOST_ID=0 FUSEE_NUM_THREADS=$t FUSEE_RUN_COOKIE=$cookie FUSEE_REP=$rep FUSEE_CACHE=$cache FUSEE_KV_SIZE=$kv FUSEE_WORKLOAD_NAME=$wl timeout $TIMEOUT_S $BIN $DEV $WL_DIR/${wl}.spec_load $WL_DIR/${wl}.spec_trans $NUM_BUCKETS $MAX_OPS"
          cmd_h1="cd /root/FUSEE_CXL/build-cxl-hazard && $COMMON_ENV FUSEE_NUM_HOSTS=2 FUSEE_HOST_ID=1 FUSEE_NUM_THREADS=$t FUSEE_RUN_COOKIE=$cookie FUSEE_REP=$rep FUSEE_CACHE=$cache FUSEE_KV_SIZE=$kv FUSEE_WORKLOAD_NAME=$wl timeout $TIMEOUT_S $BIN $DEV $WL_DIR/${wl}.spec_load $WL_DIR/${wl}.spec_trans $NUM_BUCKETS $MAX_OPS"
