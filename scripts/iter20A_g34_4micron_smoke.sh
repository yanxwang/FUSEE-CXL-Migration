#!/usr/bin/env bash
# Smoke test on g3/g4 after 4×Micron modules were remapped back to g3/g4.
# Compare to:
#   - g34 historical (iter-18A Phase 4, before module reshuffle): A=1.181 B=4.066 C=5.203 D=1.108 Mops
#   - g12 pre-remap (2×Samsung, G5x8): A=0.634 B=2.278 C=2.777 D=0.578 Mops
#   - g12 post-remap (4×Micron, G5x8): A=0.636 B=2.160 C=2.872 D=0.581 Mops

set -u
ROOT=/home/yanwang/FUSEE
TS=$(date +%Y%m%d_%H%M%S)
OUT=$ROOT/docs/iter20A_g34_4micron_smoke_$TS
mkdir -p $OUT/raw
CSV=$OUT/grid.csv
...
echo "==== g3/g4 smoke after 4×Micron remap-back ===="
for rep in 1 2 3; do
  echo "--- rep $rep ---"
  run_cell A 32 0 $rep
  run_cell B 16 4 $rep
...
g12_pre  = {"A": 0.634, "B": 2.278, "C": 2.777, "D": 0.578}
g12_post = {"A": 0.636, "B": 2.160, "C": 2.872, "D": 0.581}
print(f"{'cell':>5}{'now Mops':>12}{'g34 hist':>12}{'g12 pre':>12}{'g12 post':>12}{'vs hist':>10}")
for cell in ["A","B","C","D"]:
    sel = [float(r["thpt_Mops"]) for r in rows if r["test"]==cell]
    if not sel: continue
    med = statistics.median(sel)
    print(f"{cell:>5}{med:>12.3f}{hist[cell]:>12.3f}{g12_pre[cell]:>12.3f}{g12_post[cell]:>12.3f}{med/hist[cell]*100:>9.1f}%")
PY
echo "OUT: $OUT"
