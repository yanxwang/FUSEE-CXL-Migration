#!/usr/bin/env python3
"""iter-12A — best-throughput-per-workload summary plots.
...
def plot_thpt(best, out_path):
    ...
    ax.set_title(
        "Throughput per YCSB Workload (iter-12A best cell, 2 Hosts)")
...
def plot_lat(best, out_path):
    ...
    ax.set_title(
        "Latency per YCSB Workload (iter-12A best cell, 2 Hosts)")
...
def main():
    ...
    best = best_per_workload(rows)  # iter-12A: use overall best across all KV, no kv=1024 restriction
    ...
    THPT_OVERRIDES = {}  # No iter-11A-style supervisor overrides for iter-12A
