#!/usr/bin/env bash
# iter-5A Phase 7: scaling_ycsb sweep for Protocol A.
# Per docs/scaling_ycsb_spec.md (post-2026-05-02 single-rep default).
set -u
...
REPS=1                    # spec §3 standing default
NUM_BUCKETS=65536
MAX_OPS=200000            # spec §3 value
TIMEOUT_S=600             # bumped from 120s for invalidate-broadcast slow path
DEV=/dev/dax0.0
  ...
  for cache_mode in $CACHE_MODES; do
    ...
    for kv in $KV_SIZES; do
      for t in $THREADS; do
        for rep in $(seq 1 $REPS); do
          cookie=$(date +%s%N)
          # Best-effort cleanup between cells (per QR6).
          ...
          cmd_h1="cd /root/FUSEE_CXL/build-cxl && FUSEE_NUM_HOSTS=2 FUSEE_HOST_ID=1 FUSEE_NUM_THREADS=$t FUSEE_RUN_COOKIE=$cookie FUSEE_REP=$rep FUSEE_CACHE=$cache FUSEE_KV_SIZE=$kv FUSEE_WORKLOAD_NAME=$wl timeout $TIMEOUT_S $BIN $DEV $WL_DIR/${wl}.spec_load $WL_DIR/${wl}.spec_trans $NUM_BUCKETS $MAX_OPS"
          ...
          if [ $rc0 -eq 0 ] && [ -s /tmp/h0_out.$$ ]; then
            # Append KV size to comment field for plot scripts.
