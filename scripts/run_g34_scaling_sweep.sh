  ...
  for wl in $WORKLOADS; do
    for T in $THREADS; do
      for opt in $OPTS; do
        ...
        [ -n "${FUSEE_BATCH_NUM_FLUSHERS:-}" ] && cenv+="FUSEE_BATCH_NUM_FLUSHERS=$FUSEE_BATCH_NUM_FLUSHERS "
        [ -n "${FUSEE_VALUE_SIZE:-}" ] && cenv+="FUSEE_VALUE_SIZE=$FUSEE_VALUE_SIZE "
        [ -n "${FUSEE_SAME_HOST_BYPASS:-}" ] && cenv+="FUSEE_SAME_HOST_BYPASS=$FUSEE_SAME_HOST_BYPASS "
        [ -n "${FUSEE_PER_HOST_RING:-}" ] && cenv+="FUSEE_PER_HOST_RING=$FUSEE_PER_HOST_RING "
        [ -n "${FUSEE_SENDER_BATCH_K:-}" ] && cenv+="FUSEE_SENDER_BATCH_K=$FUSEE_SENDER_BATCH_K "
        [ -n "${FUSEE_SENDER_BATCH_T_US:-}" ] && cenv+="FUSEE_SENDER_BATCH_T_US=$FUSEE_SENDER_BATCH_T_US "
        [ -n "${FUSEE_SENDER_CORE:-}" ] && cenv+="FUSEE_SENDER_CORE=$FUSEE_SENDER_CORE "
        [ -n "${FUSEE_RECEIVER_CORE:-}" ] && cenv+="FUSEE_RECEIVER_CORE=$FUSEE_RECEIVER_CORE "
        [ -n "${FUSEE_PER_SLOT_LFM_A:-}" ] && cenv+="FUSEE_PER_SLOT_LFM_A=$FUSEE_PER_SLOT_LFM_A "
        [ -n "${FUSEE_K_CHANNELS:-}" ] && cenv+="FUSEE_K_CHANNELS=$FUSEE_K_CHANNELS "
        [ -n "${FUSEE_SENDER_CORE_BASE:-}" ] && cenv+="FUSEE_SENDER_CORE_BASE=$FUSEE_SENDER_CORE_BASE "
        [ -n "${FUSEE_RECEIVER_CORE_BASE:-}" ] && cenv+="FUSEE_RECEIVER_CORE_BASE=$FUSEE_RECEIVER_CORE_BASE "
