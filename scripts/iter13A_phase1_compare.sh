#!/usr/bin/env bash
# iter-13A Phase 1.3 — dual-track read-path comparison.
# Runs 5 representative cells × 5 reps × 3 builds (STAGING + RCU + HAZARD)
# = 75 runs. Outputs SUMMARY.tsv with (cell, build, rep) throughput/latency.
set -u
...
# 5 representative cells (per task_plan_iter13A.md)
CELLS=(
  "workloada 4 off 512"
...
for cell in "${CELLS[@]}"; do
  ...
  for build in $BUILDS; do
    if [ "$build" = "staging" ]; then bdir=build-cxl; else bdir=build-cxl-$build; fi
    ...
    for rep in $(seq 1 $REPS); do
      ...
      ARGS="FUSEE_NUM_THREADS=$T FUSEE_CACHE=$CACHE FUSEE_KV_SIZE=$KV FUSEE_WORKLOAD_NAME=$WL"
      ...
      ssh g3 "$cmd0" > /tmp/cmp_g3.$$ 2>&1 &
      p0=$!
      ssh g4 "$cmd1" > /tmp/cmp_g4.$$ 2>&1 &
      p1=$!
      wait $p0; wait $p1
      line=$(grep -E "^YCSB opt=A" /tmp/cmp_g3.$$ | tail -1)
      if [ -z "$line" ]; then
        ...
        wus=$(awk -v t="$w_avg" 'BEGIN{printf "%.3f", t/1000}')
        rus=$(awk -v t="$r_avg" 'BEGIN{printf "%.3f", t/1000}')
        wp9us=$(awk -v t="$w_p99" 'BEGIN{printf "%.3f", t/1000}')
        rp9us=$(awk -v t="$r_p99" 'BEGIN{printf "%.3f", t/1000}')
        echo -e "$build\t$WL\t$T\t$CACHE_STR\t$KV\t$rep\t$wall\t$mops\t$wus\t$rus\t$wp9us\t$rp9us" >> "$SUMMARY"
      fi
      rm -f /tmp/cmp_g3.$$ /tmp/cmp_g4.$$
      echo "[cmp] $build $WL T=$T $CACHE_STR kv=$KV rep=$rep done" >&2
    done
  done
done

echo "[cmp] all done. SUMMARY: $SUMMARY" >&2
