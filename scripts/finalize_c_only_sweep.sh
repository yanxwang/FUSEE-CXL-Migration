...
#   - runs docs/tools/plot_scaling_sweep.py to emit the 30-plot C subset
#   - runs docs/tools/plot_c_compare.py with baseline + this run for the extra/ dir
...
python3 "$HOME/FUSEE/docs/tools/plot_scaling_sweep.py" \
...
if [ ${#cmpargs[@]} -ge 4 ]; then
  python3 "$HOME/FUSEE/docs/tools/plot_c_compare.py" "$dst/extra" "${cmpargs[@]}" 2>&1 | tail -3 || true
fi
