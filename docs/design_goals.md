...
Tagging an anomaly as "noise" requires multi-rep (5 reps minimum)
verification. Skipping this step is the iter-6A precedent —
documented in `CLAUDE.md` cautionary precedent #2:

- iter-6A produced 19 collapsed cells in its 210-cell sweep
- iter-6A summary dismissed them as "single-rep timeout cascade noise"
  WITHOUT 5-rep verification
- iter-7A Phase 1 verified 22/25 collapsed cells self-resolved on
  retry — the dismissal was **substantively correct** but the
  process was **broken** (correct conclusion by accident, not by
  evidence)

**Mechanism**: `gap_to_target.md` includes a `## §13 gate 5 anomaly
scan` section auto-generated by `scripts/iter4A_redo_summarize.py`.
Sweep driver script exits non-zero if anomalies present, blocking
iter-completion declaration. iter summary doc must paste the
multi-rep numbers + median + spread for each anomaly cell, OR
acknowledge the cell as iter-N+1 backlog item.
