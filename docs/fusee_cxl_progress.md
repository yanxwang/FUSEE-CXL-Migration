...
4. **Integrate OpLog recovery callback into each CxlKvStore** — partial. C shipped in `56fb6b7` (`CxlKvStoreC::recover_from_oplog()` + `tests/cxl_kv_ops_C_recover_redo_test.cc`, green on `/dev/dax0.0`). A/B still need a `recovery_mode_` short-circuit inside `dispatch_and_wait` / `dispatch_nowait` so redo does not hang spinning on peers that have not restarted yet — deferred.
