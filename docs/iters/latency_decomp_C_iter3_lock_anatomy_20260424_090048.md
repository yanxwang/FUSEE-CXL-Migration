...
Four-panel summary generated by `docs/tools/plot_iter3_lfm_anatomy.py`:

- **(a)** Average stacked bars — `cont_wait` (red) is the only stage
  that grows with T; `local_store` / `peer_scan` / `enter_cs` are flat.
- **(b)** p50 stacked bars (first-attempt path) — acquire-physics
  (LS + PS + ECS) shrinks 4.96 µs → 3.85 µs (**−22 %**) from T=8 to
  T=86. This is the PASS-gate visual: primitive-level cost does NOT
  scale with T.
- **(c)** p99 stacked bars — `cont_wait` (red) and retry-inflated
  `local_store` (blue) dominate the tail stack. `peer_scan` /
  `enter_cs` stay compressed at the bottom even at T=86.
- **(d)** Normalised p99 growth for all 4 stages vs a linear
  reference (T / T₀). Only `cont_wait` crosses above the linear
  reference line (**11.64×** vs ref 10.75× at T=86) — the super-
  linear queueing signature. The three physics stages all stay
  **below linear** (local_store 7.9×, enter_cs 6.6×, peer_scan
  5.4×); their growth is pure retry-inflation on the p99 tail,
  not algorithmic cost-per-attempt.
