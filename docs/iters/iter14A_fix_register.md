...
| **F2** | P4 | `cache_pool_lookup` writes `lru_epoch` on every hit → MESI ping-pong on KvCacheEntry cacheline 0; sample 1/64 hits via `__rdtsc() & 0x3F == 0` to preserve approximate-LRU semantics | 6 | workloadc T=64 c=on kv={1024,256}, workloadb T=64 c=on kv=1024 | workloada/d T=64 c=on kv=1024 | (per cell) | (per cell) | (per cell) | target +5% med, CI lo > +1% | **ROLLBACK** | First measurement attempt: flag declared but not wired in `cache_pool.cc` — null compare. Real implementation rebuilt + remeasured: deltas -1.08% to +2.38%, all CIs cross 0. 0/3 targets reach +1% CI-lo threshold. R2hit anomaly is real (probe data 7.7× spec) but not throughput-load-bearing — W10 (cache_pool_insert MESI on 1088B entry) + bimodal flat dominate. Flag default OFF; code kept. Detail: [docs/iter14A_p4_f2_lru_sample_real_20260519_025549/decision.md](../iter14A_p4_f2_lru_sample_real_20260519_025549/decision.md). iter-15A: re-measure F2 after W10 structural fix. |
